  ...
  return(
    <>
      ...
      <dialog style={{position:'absolute', top: `${click.y}px`, left:`${click.x}px`, marginLeft:'0px',zIndex:'2'}}open={isModalOpen}>
        <menu>
          <button onClick={(e)=>{setSelection({click},e.value)}}>Cat</button>
          <button onClick={(e)=>{setSelection({click},e.value)}}>Dog</button>
          <button onClick={(e)=>{setSelection({click},e.value)}}>Crayons</button>
          <button onClick={(e)=>{setSelection({click},e.value)}}>Book</button>
          <button onClick={(e)=>{setSelection({click},e.value)}}>Doll</button>
        </menu>
      </dialog>
    </>
  );
